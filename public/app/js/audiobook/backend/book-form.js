...
$(document).ready(() => {
    ...
    $("#bookmodal").on('hidden.bs.modal', e => {
        ...
        $('.b_book').text('*Note: Audio size should not be more then 200 MB');
        tags = [];
        $('#bookmodal').removeClass('rtl').css('text-align', 'left');
        $('#lbl_language').text('زبان');
    ...
    $('span.book_tag').click(e => {
        ...
        if (element.length < 5) {
            if ($(element).attr('class') == $(e.target).attr('class')) {
                ...
                $('span#tagcounter').text(element.length - 1)
                ...
                $(e.target).toggleClass('bg--two')
                $(e.target).toggleClass('text-white')
                $('span#tagcounter').text($('span.book_tag.bg--two').length)
    ...
    $('#bookform').ajaxForm({
            ...
            complete: function(result) {
                ...
                $('.b_cover').text('*Note: Image must be between 500 KB - 2 MB');
                $('.b_cover').removeClass('text-danger');
                $('.b_book').text('*Note: audio size should not be more then 60 MB');
                tags = [];
    ...
    $('#b_book').change(d => {
        if (d.target.value != '') {
            ...
            if (image == 'mp3') {
                ...
                if (size > 60000000) {
                    $('.b_book').text('mp3 size should not be more then 200 MB')
                ...
                $('.b_book').text('Only mp3 Format is Allowed!');
                $('.b_book').addClass('text-danger');
                return false;
            }
...
function validate() {
    ...
    if ($('#b_book').val() == '') {
        ...
        let mp3 = $('#b_book').val().split('.').pop().toLowerCase();
        if (mp3 == 'mp3') {
            let size = $('#b_book')[0].files[0].size;
            if (size > 200000000) {
                if (language == 'english')
                    $('.b_book').text('Audio size should not be more then 200 MB').addClass('test-danger');
                else if (language == 'pashto')
                    $('.b_book').text('صوتی سایز له ۲۰۰ mb زیاد اجازه نه ده').addClass('test-danger');
                else
                    $('.b_book').text('سایز صوتی از ۲۰۰ mb زیاد بوده نمی تواند').addClass('test-danger');
            ...
            if (language == 'english')
                $('.b_book').text('Only mp3 Format is Allowed!').addClass('text-danger');
            else if (language == 'pashto')
                $('.b_book').text('یوازی mp3 فارمت اجازه ده').addClass('text-danger');
            else
                $('.b_book').text('تنها mp3 فارمت اجازه است').addClass('text-danger');
            return false;
        }
