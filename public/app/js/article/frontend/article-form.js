...
$(document).ready(() => {
    ...
    $("#articlemodal").on('hidden.bs.modal', e => {
        ...
        $.each($('#a_catagory>option').not('#a_cat_lbl'), (index, span) => {
...
function a_dari(lang) {
    ...
    $('#a_cat_lbl').text('انتخاب کتگوری');
    $('#lbl_a_tag').text('برچسپ')
...
function a_english(lang) {
    ...
    $('#a_lblarticle').text("Uploaded : ");
    $('#a_lang').text('Select Language');
    $('#a_cat_lbl').text('choose category');
    $('#lbl_a_tag').text('Tags')
...
function a_pashto(lang) {
    ...
    $('#lbl_a_page').text('دفصل او پانه نمبر')
